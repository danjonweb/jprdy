...
{input:"abs",   tag:"mo", output:"abs",  tex:null, ttype:UNARY, rewriteleftright:["|","|"]}, // ADDED
...
//tokens are single letters (except a, A, I) and ASCIIMathML tokens
  ...
  var englishAMtoken = "\\b(?:sum|ox|log|sin|tan|dim|hat|bar|dot|line|arc)(?![a-z])|";
...
{input:"\\line",		tag:"mover",  output:"\u2194", ttype:UNARY, acc:true},
{input:"\\arc",		tag:"mover",  output:"\u23DC", ttype:UNARY, acc:true},
...
//	  node = createMmlNode("mrow",node);  // hopefully unnecessary
	...
	if (isIE) {
		if (symbol.input == "\\hat")
			...
			output = "\u00AF";
		else if (symbol.input == "\\line")
			output = "\u2194";
		else if (symbol.input == "\\arc")
			output = "\u23DC";
	...
	if (isIE && symbol.input == "\\line")
	    node1.setAttribute("maxsize","1.2");
	if (isIE && symbol.input == "\\arc")
		node1.setAttribute("maxsize","1.2");
